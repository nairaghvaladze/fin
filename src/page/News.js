  ...
  .header .menu-icon {
    display: none;
        ...
        <div class="div2">
          ...
          <div>
          <form id="editForm" onsubmit="submitEditForm(${id}); return false;">
                  <label for="editTitle" class="saxeli">სათაური:</label>
                  <input type="text" id="editTitle" value="${title}" required />

                  <label for="editDescription" class="saxeli">შინაარსი:</label>
                  <textarea id="editDescription" required>${description}</textarea>

                  <button type="submit" class="edit">რედაქტირება</button>
                </form>

          </div>
          <div>
          <button onclick="deleteNews(${id})" class="delete" >პოსტის წაშლა</button>
          </div>
          </div>
          <div>
        ...
        function submitEditForm(postId) {
          ...
          axios.put(\`https://apitest.reachstar.io/blog/edit/${id}\`, {
            ...
            .then((response) => {
              console.log('პოსტი წარმატებით დარედაქტირდა', response.data);
        ...
        function deleteComment(id) {
          axios.delete(\`https://apitest.reachstar.io/comment/delete/${id}\`)
              .then(response => {
                  console.log('წარმატებით წაიშალა', response.data);
                 
              })
              .catch(error => {
                  console.error('შეცდომაა წაშლისას', error);
